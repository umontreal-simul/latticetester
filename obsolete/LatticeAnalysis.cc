...
namespace LatticeTester
{
  ...
  {   
    ...
    LatTestWriter<MScal>* rw = createLatTestWriter (infile, config.outputType);

    // creating the Reducer object from input
    IntLatticeBasis<MScal> basis (config.basis, config.dim, config.norm);
    Reducer red (basis);
    // update parameters
    setReducer(red);
  ...
  LatTestWriter<MScal>* LatticeAnalysis::createLatTestWriter (const char *infile, OutputType ot)
  {
    LatTestWriter<MScal> *rw = 0;
    string fname;
    ...
    switch (ot) {
      case RES:
        fname = infile;
        fname += ".res";
        rw = new LatTestWriterRes<MScal> (fname.c_str ());
        break;
      ...
      case TERMINAL:
        rw = new LatTestWriterRes<MScal> (&cout);
        break;

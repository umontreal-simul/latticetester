...
namespace LatticeTester {
      ...
      PrecisionType doublePrecision, double fact, int blocksize)
  {
    ...
    IntLatticeBasis<MScal> basis (matrix, dimension, norm);
    Reducer<MScal> red (basis);
  ...
  double ShortestVector(BMat matrix, NormType norm, long maxNodesBB, PreReductionType preRed, 
      PrecisionType doublePrecision, double fact, int blocksize)
  {
    Reducer<MScal>::maxNodesBB = maxNodesBB; // setting the number of nodes visited in the BB procedure
      ...
      double fact, int blocksize)
  {
    ...
    // creating objects needed to perform the test
    IntLatticeBasis<MScal> basis (matrix, dimension, L2NORM);
    Reducer<MScal> red (basis);
